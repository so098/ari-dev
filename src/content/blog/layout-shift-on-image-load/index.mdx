      ...
      <div className="grid w-full grid-cols-5 gap-4 p-4">
        {loading
          ? new Array(30)
              .fill(0)
              .map((_, i) => (
                <div key={i} className="aspect-square w-full animate-pulse bg-gray-300"></div>
              ))
          : gallery.map((item) => (
...
<div class="flex  gap-4 my-4">
  <div class="border rounded-xl px-3 py-5 bg-white">
	...
	<p class="text-center text-[0.7rem] text-gray-600 mb-3  mt-0">Cumulative Layout Shift (CLS) 점수</p>
  ...
  <div class="border rounded-xl p-3 bg-white px-3 py-5">
